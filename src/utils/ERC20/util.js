...
import { DUMMY_ADDRESS, DUMMY_ADDRESS_2 } from '../../utils/constants';
import { convertString, toFloat, unpackResult } from '../tools';

export async function getAllowancesFromApprovals(contract, ownerAddress, approvals) {
  const deduplicatedApprovals = approvals.filter(
    (approval, i) => i === approvals.findIndex((other) => approval.topics[2] === other.topics[2])
  );

  let allowances = await Promise.all(
...
async function getAllowanceFromApproval(multicallContract, ownerAddress, approval) {
...
export async function getTokenData(contract, ownerAddress, tokenMapping = {}) {
...
export function formatAllowance(allowance, decimals, totalSupply) {
...
async function throwIfNotErc20(contract) {
